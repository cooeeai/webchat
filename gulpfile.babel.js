...
gulp.task('copy:html', () =>
  gulp.src(paths.src.html).pipe(gulp.dest(paths.dest.target))
);
...
gulp.task('webpack', (done) => {
  ...
  const configFile = process.env.NODE_ENV === 'production' ? config.webpack.production : config.webpack.development;
...
gulp.task('webpack:development', gulp.series((done) => {
  process.env.NODE_ENV = 'development';
  done()
}, 'webpack'));

gulp.task('webpack:production', gulp.series((done) => {
...
gulp.task('serve', (done) => {
  ...
  browserSync.create().init(config.browserSync, done);
});
